...
def convert_currency(sender_value: str, receiver_value: str, amount: float = 1):
    if sender_value == receiver_value:
        return amount
    try:
        result = requests.get(f"https://api.apilayer.com/fixer/convert?to={receiver_value}"
...
def get_all(search: str = None):
    if search:
        data = read_query("SELECT * FROM currency WHERE name = ?", (search,))
        return next((Currency.from_query(*row) for row in data), None)
    else:
        data = read_query("SELECT * FROM currency")
        return (Currency.from_query(*row) for row in data)


def get_by_id(id: int):
    data = read_query("SELECT * FROM currency WHERE id = ?", (id,))

    return next((Currency.from_query(*row) for row in data), None)


def create(name: str):
    data = insert_query("INSERT INTO currency(name) VALUES(?)", (name,))
    return data
